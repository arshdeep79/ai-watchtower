...
import { useSetIsDrawing, useClearDrawnFeatures, useMapStore } from '../store/mapStore';
...
const DrawingControls: React.FC<DrawingControlsProps> = ({ draw, map }) => {
  ...
  const { isDrawing } = useMapStore();

  const toggleDrawingMode = (mode: 'draw_polygon') => {
    if (draw.current && map.current) {
      if (isDrawing) {
        // If already drawing, switch back to simple_select mode
    ...
    <div className="absolute top-4 left-4 bg-white rounded shadow-md shadow-black/20 z-10">
      <div className="flex flex-col">
        <button
          ...
          className={`p-2 transition-colors border-b border-gray-200 rounded-t ${
            isDrawing 
